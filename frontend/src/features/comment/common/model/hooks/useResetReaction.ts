...
import type { ReactionProps } from "../types/reaction";

export const useResetReaction = ({
  userId,
  entityType,
  entityId,
}: ReactionProps) => {
  ...
  const updateRecursive = (
    ...
    return comments.map((comment) => {
      if (comment.id === targetId) {
        const updatedLikes = (comment.likes || []).filter((id) => id !== uid);
  ...
  return useMutation({
    mutationFn: async (commentId: string) => {
      ...
      return commentApi.clearReaction(commentId);
    },
