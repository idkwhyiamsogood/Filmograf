...
public class CollectionsHistoryBuildingService
{
    ...
    public CollectionsHistoryBuildingService(UserCollectionsActivityDailyRepository activityRepository)
    {
        _activityRepository = activityRepository;
    }

    private async Task<List<UserCollectionClickEvent>> ListUserClicksAsync(Guid userId, PaginationQueryDto pagination,
    ...
    public async Task<List<string>> HandleBuildHistoryAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        return history.Select(item => item.CollectionId).ToList();
    }
}